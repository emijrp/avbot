	...
	def on_pubmsg(self, c, e):
		...
		if re.search(avbotglobals.parserRegexps['edit'], line):
			...
			for m in match:
				...
				if avbotglobals.preferences['language']=='pt' and re.search(ur'(?i)(Revertidas edições por.*%s.*para a última versão por|Desfeita a edição \d+ de.*%s)' % (avbotglobals.preferences['botNick'], avbotglobals.preferences['botNick']), editData['resume']) and editData['pageTitle']!='Usuario:AVBOT/Errores/Automático':
					if not avbotglobals.preferences['nosave']:
						...
						wiii.put(u'%s\n# [[%s]], {{subst:CURRENTDAY}} de {{subst:CURRENTMONTHNAME}} de {{subst:CURRENTYEAR}}, http://%s.wikipedia.org/w/index.php?diff=%s&oldid=%s, {{u|%s}}' % (wiii.get(), editData['pageTitle'], avbotglobals.preferences['language'], editData['diff'], editData['oldid'], editData['author']), u'BOT - Aviso automático. [[User:%s|%s]] reverteu [[User:%s|%s]] em [[%s]]' % (editData['author'], editData['author'], avbotglobals.preferences['botNick'], avbotglobals.preferences['botNick'], editData['pageTitle']))
			...
			for m in match:
				...
				block=m.group('block')
				wikipedia.output(u'\03{lightblue}Registro combinado: [[User:%s]] (%d) foi bloqueado por [[User:%s]] (%d) por prazo de %s\03{default}' % (blocked, len(blocked), blocker, len(blocker), block))
