		...
		if editData['userClass']=='anon' or (editData['userClass']=='reg' and avbotglobals.userData['edits'][editData['author']]<=avbotglobals.preferences['newbie']):
			...
			if destruir:
				...
				if not avbotglobals.preferences['nosave']:
					editData['page'].put(u'{{RoboER|%s|%s}}\n%s' % (editData['author'], motive, editData['newText']), u'ER. Motivo: %s. Página criada por [[User:%s|%s]] ([[User talk:%s|disc]] · [[Special:Contributions/%s|cont]])' % (motive, editData['author'], editData['author'], editData['author'], editData['author']))
				return True, motive
	return False, motive
			...
			if not re.search(ur'(?i)\{\{ *(destruir|plagio|copyvio|vda|er)|redirect', newText): #descarta demasiado? destruir|plagio|copyvio
				if len(newText)>=500:
					...
					if len(newnewText)>len(newText):
						if not avbotglobals.preferences['nosave']:
							editData['page'].put(newnewText, u'BOT - A adicionar %s... ao novo artigo' % resumen)
						return True, resumen
	return False, u''
