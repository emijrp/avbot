...
def msgVandalismoEnCurso(dic_vand, author, userclass, blockedInEnglishWikipedia):
	...
	for k, v in dic_vand.items():
		if k!='avisos':
			...
			explanation+=u"[http://pt.wikipedia.org/w/index.php?title=%s&diff=%s&oldid=prev %s], " % (wtitle_, k, wtitle)
	wikipedia.output(u"O usuário %s cometeu vários vandalismos" % author)
	wii=wikipedia.Page(avbotglobals.preferences['site'], u"Pedidos a administradores/Pedidos de bloqueio")
	restopag=wii.get()
	...
	if userclass=='reg':
		...
		if blockedInEnglishWikipedia[1]: #esta bloqueado en la inglesa y es proxy?
			...
			resume+=u". Bloqueado na Wikipedia em inglês"
			explanation+=u" (Posivel proxy)"
			resume+=u" (Posible proxy)"
		report+=u'<!-- completa los datos tras las "flechitas" -->\n{{subst:bloquear\n| 1 = %s\n| 2 = %s\n}}' % (author, explanation)
	if not avbotglobals.preferences['nosave']:
		wii.put(u'%s\n\n%s' % (restopag, report), u'BOT - Adicionando pedido de bloqueio de [[Special:Contributions/%s|%s]]%s' % (author, author, resume))
...
def sendMessage(author, wtitle, diff, n, tipo):
	...
	wtitle2=wtitle
	wtext=u""
	if re.search(ur'(?i)Categor(ia|y)\:', wtitle2):
		wtitle2=u':%s' % wtitle2
	...
	if avisotexto:
		...
		wtext+=avisotexto
		if not avbotglobals.preferences['nosave']:
			talkpage.put(wtext, u"BOT - Avisando [[Special:Contributions/%s|%s]] de que a sua edição em [[%s]] foi revertida (Aviso #%d)" % (author, author, wtitle, n))
...
def msgBlock(blocked, blocker):
	...
	if not avbotglobals.preferences['nosave']:
		aviso.put(avisotexto, u"BOT - Avisando a [[Special:Contributions/%s|%s]] de que foi bloqueado por [[User:%s|%s]]" % (blocked, blocked, blocker, blocker))
