...
def loadMessages():
	...
	p=wikipedia.Page(avbotglobals.preferences['site'], u'User:Alchimista/Mensagens.css') #Fijo a Emijrp para los clones de es:
	raw=''
	if p.exists():
...
def loadExclusions():
	...
	p=wikipedia.Page(avbotglobals.preferences['site'], u'User:Alchimista/exclusões.css')
	raw=''
	if p.exists():
		if not p.isRedirectPage() and not p.isDisambig():
			raw=p.get()
